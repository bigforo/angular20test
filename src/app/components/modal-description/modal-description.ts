...
@Component({
  ...
  template: `
    <ion-modal [isOpen]="open()" [canDismiss]="canDismiss" (willDismiss)="onWillDismiss($event)" [initialBreakpoint]="1" [breakpoints]="[0, 0.75, 1]">
      <ng-template>
        <ion-header>
          <ion-toolbar>
        ...
        <ion-content class="ion-padding">
          <ion-textarea mode="md" (ionChange)="change($event)" [value]="appState().current?.note" label="Session/Workout Notes" label-placement="floating" fill="outline" rows="10" />
        </ion-content>
      </ng-template>
    </ion-modal>
  `,
